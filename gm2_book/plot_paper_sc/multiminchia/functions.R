...
plot_all_fits <- function(name, Wind, quark, myyrange = NULL, quark_full_name = NULL, to_plot = TRUE,
  ...
  gg<-gg+ geom_histogram(
    ...
    # n = 1000
  )+scale_x_log10() + coord_cartesian(ylim = myyrange)
  # myplotly(gg," "," "," ",save_pdf = "condition number",legend_position = NULL)
  ggsave(paste0("condition_number_",name,"_",quark,"_.png"),gg)
  
  ggk <- ggplot()+theme_bw()+ylab("continuum value") +xlab("k")
  tresh<-1e+4
  df$color<- (df$k<tresh)
  ggk<- ggk+geom_errorbar(
    # adjust = 0.3,
    aes(
      y = df$res,
      xmin =0*df$k,
  ...
  ggk
  ggsave(paste0("x_vs_kappa_",name,"_",quark,"_.png"),ggk)
