...
e <- c("D96", "D96_184confs", "D96_122confs")
confs <- c(256, 184, 122)
...
df <- add_corr_to_df(string =name, all_obs, mt, df,
  reshape = FALSE, nudge = 0.0+(i-1)*0.2,print_res = FALSE, rename = paste0(name,confs[i])
)
name<-"amu_{bound}_(TM)_below"
df <- add_corr_to_df(name, all_obs, mt, df,
  reshape = FALSE, nudge = 0.1+(i-1)*0.2, rename = paste0(name,confs[i])
)
name<-"amu_{bound_meff_t}_(TM)_below"
df <- add_corr_to_df(name, all_obs, mt, df,
  reshape = FALSE, nudge = 0.05+(i-1)*0.2, rename = paste0(name,confs[i])
)
name<-"amu_{bound_meff}_(TM)_above"
df <- add_corr_to_df(name, all_obs, mt, df,
  reshape = FALSE, nudge = 0.15+(i-1)*0.2, rename = paste0(name,confs[i])
)
}
###############################################################################
