...
int main(int argc, char** argv) {
    ...
    // fit_info.compute_cov1_fit();
    fit_info.guess = { 0.0908026, 0.07951 , 0.06816, 0.05688, 0.04891 ,-8.75 ,-500 };
    mysprintf(namefit, NAMESIZE, "afpi_A12_noC20_cov_unitary");
    fit_result fit_afpi_A12_noC20_unitary = fit_all_data(argv, jackall, lhs_afpi_max_twist, fit_info, namefit);
    fit_info.band_range = { 0.005,0.035 };
    // std::vector<double> xcont = { 0, 0 /*Delta*/, 0, 0,/*l, a,m*/ fit_info.x[4][0][Njack - 1],
    //      fit_info.x[5][0][Njack - 1] , fit_info.x[6][0][Njack - 1], fit_info.x[7][0][Njack - 1] };
    ...
    //    Mpi:   the index of the parameter do not match!   P[i]*(M_pi- M_pi_phys ) 
    // std::vector<double> xcont = {};
    print_fit_band(argv, jackall, fit_info, fit_info, namefit, "xi", fit_afpi_A12_noC20_unitary, fit_afpi_A12_noC20_unitary, 4, 0, 0.0005, xcont);

    myres->write_jack_in_file(fit_afpi_A12_noC20_unitary.P[0], "../../g-2_new_stat/out/a_fm_A_A12_noC20_unitary.txt");
    myres->write_jack_in_file(fit_afpi_A12_noC20_unitary.P[1], "../../g-2_new_stat/out/a_fm_B_A12_noC20_unitary.txt");
    myres->write_jack_in_file(fit_afpi_A12_noC20_unitary.P[2], "../../g-2_new_stat/out/a_fm_C_A12_noC20_unitary.txt");
    myres->write_jack_in_file(fit_afpi_A12_noC20_unitary.P[3], "../../g-2_new_stat/out/a_fm_D_A12_noC20_unitary.txt");
    myres->write_jack_in_file(fit_afpi_A12_noC20_unitary.P[4], "../../g-2_new_stat/out/a_fm_E_A12_noC20_unitary.txt");
...

    {
        ...
        for (int n = 0;n < fit_info.Nxen.size();n++) {
            for (int e : fit_info.Nxen[n]) {
                for (int j = 0;j < Njack;j++) {
                    ...
                    if (e == A53 || e == A40 || e == A30 || e == A12)
                        fit_info.x[8][count][j] = fit_afpi_A12_noC20_unitary.P[0][j];
                    else if (e == B25_48 || e == B14_64 || e == B72_64 || e == B72_96)
                        fit_info.x[8][count][j] = fit_afpi_A12_noC20_unitary.P[1][j];
                    else if (e == C20 || e == C06 || e == C112)
                        fit_info.x[8][count][j] = fit_afpi_A12_noC20_unitary.P[2][j];
                    else if (e == D54)
                        fit_info.x[8][count][j] = fit_afpi_A12_noC20_unitary.P[3][j];
                    else if (e == E112)
                        fit_info.x[8][count][j] = fit_afpi_A12_noC20_unitary.P[4][j];
                    else { printf("error missing ensemble\n"); exit(1); }
        ...
        // fit_info.guess = { 1, 1 , 1, 1  };
        // fit_info.guess = { 0.05 };
        mysprintf(namefit, NAMESIZE, "aMpi2_over_afpi2_A12_noC20_cov_unitary");
        fit_result fit_aMpi2_over_afpi2 = fit_all_data(argv, jackall, lhs_Mpi2_over_afpi2_max_twist, fit_info, namefit);
        fit_info.band_range = { 0.00001,0.0055 };
    ...
    //////////////////////////////////////////////////////////////
    // Mpi with A12, max twist corrections, noC20, stange misstuning, unitary
    //////////////////////////////////////////////////////////////
