...
int main(int argc, char** argv) {
    ...
    double mus = atof(argv[11]);
    std::vector<std::string> string_mus = { argv[11] };
    double muc1 = atof(argv[12]);
    double muc2 = atof(argv[13]);
    double muc3 = atof(argv[14]);

    std::vector<std::string> string_muc = { argv[12],argv[13], argv[14] };
    ...
    mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%s_mu.%.5f_P5P5.txt", argv[3], argv[4], string_mus[0].c_str(), muc3);//0
    correlators.emplace_back(namefile);

    // mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%.5f_VKVK.txt", argv[3], argv[4], mus, string_muc);//2
    // correlators.emplace_back(namefile);
    // }
    ...
    double* M_Ds2 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 1, "M_{Ds}^{s,c2}", M_eff_T, jack_file);
    check_correlatro_counter(2);

    double* M_Ds3 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 2, "M_{Ds}^{s,c3}", M_eff_T, jack_file);
    check_correlatro_counter(3);


    double** MDs = (double**)malloc(sizeof(double*) * string_muc.size());
    MDs[0] = M_Ds1;
    MDs[1] = M_Ds2;
    MDs[2] = M_Ds3;

    double** mc = (double**)malloc(sizeof(double*) * string_muc.size());
    mc[0] = fake_sampling(resampling, header.mus[1], 1e-10, Njack, 1);
    mc[1] = fake_sampling(resampling, header.mus[2], 1e-10, Njack, 1);
    mc[2] = fake_sampling(resampling, header.mus[3], 1e-10, Njack, 1);
    ///  ms from MDs
