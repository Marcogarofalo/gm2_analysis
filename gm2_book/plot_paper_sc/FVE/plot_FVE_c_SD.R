...
mydata <- c("OS", "TM")
# gg <- plot_fit(
basename <- "/home/garofalo/analysis/g-2_new_stat/fit_all_charm/amu_SDcor_3b_BOS_BTM_FVE_a4OS_a4TM"
var <- "afm"
id_x <- 1
data_type <- mydata
width <- 1e-4
...
ylab <- paste0("$a_{\\mu}^{\\rm HVP, SD}(c,L)/a_{\\mu}^{\\rm HVP, SD}(c,L')$")
nameout <- paste0("amu_FVE_c_SD_ratio")
