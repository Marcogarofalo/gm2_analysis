...
gg <- gg + geom_pointrange(aes(
  x = fit$P[c(1:5), 2]^2, y = fit$P[c(1:5), 2],
  ymin = fit$P[c(1:5), 2] - fit$P[c(1:5), 3],
  ymax = fit$P[c(1:5), 2] + fit$P[c(1:5), 3],
  color = "rew",
  shape = "rew",
  fill = "rew"
))
...
fig <- myplotly(gg, "", "$a^2$", "$a(loop)-a(rew)$", to_print = TRUE, xrange = c(0, 0.0085))
```
