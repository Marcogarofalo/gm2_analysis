...
eq_op1<-function(x){
  if(x==0) return("OS-a(tau)") # equal
  else return("TM-a(tau)") # opposite
}
...
# }
gg<-myggplot()
gg<-plot_fit(basename = "/home/garofalo/analysis/g-2_new_stat/fit_all/amu_full_a2_a4_MK_cov", var="afm",data_type = c("TM","OS"), width=1e-4, gg=gg
...
gg<-plot_fit(basename = "/home/garofalo/analysis/g-2_new_stat/fit_all_a_from_tau/amu_full_a2_a4_MK_cov", var="afm",data_type = c("TM-a(tau)","OS-a(tau)"), width=1e-4, gg=gg
                 ,               id_color = iy+2, id_shape = iy+2,
             # , labelfit = "zap"
...
fig<- myplotly(gg,"","$a^2(\\mbox{fm})$", "$a_{\\mu}(s)$", to_print = TRUE
