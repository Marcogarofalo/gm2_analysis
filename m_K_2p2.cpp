...
int main(int argc, char** argv) {
    ...
    std::vector<std::string> string_mus = { argv[11], argv[12], argv[16] };


    double mul1 = atof(argv[14]);
    double mul2 = atof(argv[15]);

    double mus = atof(argv[16]);

    std::vector<std::string> string_mul = { argv[14], argv[15], argv[13] };
    // double mus1 = atof(argv[12]);
    // double mus2 = atof(argv[13]);

    // double mul1 = atof(argv[14]);
    // double mul2 = atof(argv[15]);
    // double mul3 = atof(argv[16]);
    ...
    header.mus = { mus1 , mus2, mul,  mul1, mul2, mus };
    // if (argc > 17 && strcmp(argv[17], "three_corr") != 0)  header.mus.emplace_back(mul1);
    header.thetas = {};

    mysprintf(namefile, NAMESIZE, "%s_K", argv[4]);
    ...
    mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%s_mu.%s_P5P5.txt", argv[3], argv[4], string_mul[2].c_str(), string_mus[0].c_str());//0
    correlators.emplace_back(namefile);

    mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%s_mu.%s_P5P5.txt", argv[3], argv[4], string_mul[2].c_str(), string_mus[1].c_str());//0
    correlators.emplace_back(namefile);

    mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%s_mu.%s_P5P5.txt", argv[3], argv[4], string_mul[0].c_str(), string_mus[2].c_str());//0
    correlators.emplace_back(namefile);

    mysprintf(namefile, NAMESIZE, "%s/%s_r.opposite_mu.%s_mu.%s_P5P5.txt", argv[3], argv[4], string_mul[1].c_str(), string_mus[2].c_str());//0
    correlators.emplace_back(namefile);
    ...
    double* M_K1 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 0, "M_{K1}", M_eff_T, jack_file);
    check_correlatro_counter(1);

    double* M_K2 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 1, "M_{K2}", M_eff_T, jack_file);
    check_correlatro_counter(2);

    double* M_K3 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 2, "M_{K3}", M_eff_T, jack_file);
    check_correlatro_counter(3);

    double* M_K4 = plateau_correlator_function(option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack, namefile_plateaux, outfile, 3, "M_{K4}", M_eff_T, jack_file);
    ...
    jackall.en[0].jack[0] = M_K1;
    jackall.en[1].jack[0] = M_K2;
    jackall.en[2].jack[0] = M_K3;
    jackall.en[3].jack[0] = M_K4;
    ...
    fit_info.N = 1;
    int offset = 6;
    fit_info.myen = { 0,1,2,3 };
    fit_info.entot = fit_info.myen.size() * fit_info.N;
    fit_info.malloc_x();
    count = 0;
    for (int j = 0;j < fit_info.Njack;j++) {
        fit_info.x[0][0][j] = mul; // mul1 
        fit_info.x[1][0][j] = mus1; // mul2

        fit_info.x[0][1][j] = mul; // mul1 
        fit_info.x[1][1][j] = mus2; // mul2

        fit_info.x[0][2][j] = mul1; // mul1 
        fit_info.x[1][2][j] = mus; // mul2

        fit_info.x[0][3][j] = mul2; // mul1 
        fit_info.x[1][3][j] = mus; // mul2
    }
    count++;
    ...
    myres->mult(jack_aMK2_exp, jack_aMK2_exp, a);
